...
async function Home() {
    ...
    const secciones = recetas.reduce((acc, receta) => {
        const seccion = acc.find(s => s.nombre === receta.Nombre) //DEBERIA CAMBIAR POR CATEGORIA
        if (seccion) {
            seccion.recetas.push(receta)
        } else {
            acc.push({ nombre: receta.Nombre, recetas: [receta] })
        }
        return acc
    }, [] as { nombre: string; recetas: Receta[] }[])
