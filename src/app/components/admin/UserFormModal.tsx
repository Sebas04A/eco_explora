...
import { UsuarioFormData, Rol } from '@/app/typesa/admin-types';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
        <h2 className="text-2xl font-bold mb-6">{userData ? 'Editar Usuario' : 'Crear Usuario'}</h2>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-gray-700 mb-2" htmlFor="Nombre">Nombre</label>
            <input type="text" name="Nombre" id="Nombre" value={formData.Nombre} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg" required />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 mb-2" htmlFor="Correo">Correo Electrónico</label>
            <input type="email" name="Correo" id="Correo" value={formData.Correo} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg" required />
          </div>
          {/* 5. Reemplazamos el input de Rol por un select (combobox) */}
          <div className="mb-4">
            <label className="block text-gray-700 mb-2" htmlFor="RolID">Rol</label>
            <select name="RolID" id="RolID" value={formData.RolID} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg bg-white">
              {roles.map(rol => (
           ...
           <div className="mb-4">
            ...
            <input type="password" name="Contrasena" id="Contrasena" value={formData.Contrasena || ''} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg" placeholder={userData ? 'Dejar en blanco para no cambiar' : ''} />
          </div>
