...
async function ListaForo({ nombrePlanta }: { nombrePlanta: string }) {
    ...
    const forosFiltrados = todosLosForos.filter(foro => {
        ...
        return (
            foro.Planta?.trim().toLowerCase() === nombrePlanta?.trim().toLowerCase()
        )
    })

    return (
        <div>
            <h2 className='text-2xl font-bold text-gray-800 mb-4'>Foro de la Comunidad</h2>
            
            {forosFiltrados.length === 0 ? (
                <div className='text-center text-gray-600 bg-white/50 p-6 rounded-lg'>
                    <p>Aún no hay entradas en el foro para esta planta.</p>
